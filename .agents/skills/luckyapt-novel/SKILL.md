...
### ★모델 독립 품질 게이트 (2026-07-04 작가 지시 — 어떤 모델이든 동일 적용)
> **품질의 하한은 모델의 지능이 아니라 게이트의 실행이 보장한다.** Opus/Sonnet 등 어떤 모델로 실행하든, 아래 게이트는 '읽고 참고'가 아니라 **실행·기록 의무**다. 게이트를 생략한 산출물은 무효로 간주하고 재실행한다. 판단이 필요한 항목은 감(感)이 아니라 정본 문서의 명시 기준(체크리스트·수치·금지 목록)과 측정 결과에 근거해 답한다.
> 1. **프리플라이트(전 에이전트, 작업 시작 전):** 자기 정의 파일의 '입력' 목록에 있는 정본을 실제로 열었는지 확인 — 특히 12_canon(인물)·06(흡입력)·07 §0~§0-C(디렉티브)·09(떡밥 원장). 존재하지 않는 경로면 진행하지 말고 보고.
> 2. **저장 전 게이트(prose-writer):** 7-체크(엔딩 순환·웃음 비트·워치리스트 상한·사이다 비트·분량 측정·변주 체크·매력 조연) 전 항목에 답하고, 분량 측정 명령을 **실제 실행**해 수치를 리포트에 남긴다.
> 3. **검수 게이트(continuity-editor):** 축 A(정합)·B(개연)·C(흡입력) 전 항목을 회차마다 명시적으로 답한다("문제없음"도 기록). pattern_check.py를 실행해 결과를 첨부한다.
> 4. **윤문 게이트(humanizer):** 매 회차 강제 체크 5항(봄볕 엔딩·유머 밀도·워치리스트·구조/대화 반복·매력 변별)을 작업 원칙보다 먼저 실행하고, 윤문 전/후 분량을 측정해 하한(5,200) 사수를 증명한다.
> 5. **원장 게이트(plot-architect):** 회차 시놉시스에 떡밥 ID(심기/상기/회수)를 명시하고 09 원장을 동기 갱신한다.
...
- **★ 미러 동기화(하네스 파일 한정):** 정본은 `.claude/`(agents·skills). 수정 시 Codex용 미러를 갱신한다 — ①`.claude/skills/*` → `.agents/skills/*` `cp` 복사, ②`.claude/agents/*.md` → `.codex/agents/*.toml` 재생성(md 본문 내장 + Codex 실행 노트 푸터 — 재생성 스크립트는 harness-changelog 참조), ③`AGENTS.md`는 CLAUDE.md 사본이 아니라 **Codex 전용 라우터**(스킬을 파일 경로로 안내) — 하네스 구조 변경 시에만 손으로 갱신. 미러를 직접 수정하지 말 것.
