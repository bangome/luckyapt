...
- **반환 규약(공통):** 모든 전문 에이전트는 산출물을 약속된 파일에 쓴 뒤 → wrighting 동기화 → 최종 응답 메시지에 핵심 요약(산출·발견·변경 + 파일 경로)을 함께 반환한다.
- **★ wrighting 동기화 원칙 (전 에이전트 공통·필수):** 모든 산출물은 `_workspace/` 저장 즉시 wrighting에도 동기화한다.
